...
**Left click** to add a positive (green) point.
**Right click** on empty canvas to add a negative (red) point.
**Right click on a point** to delete it.
...
"""
    ...
    def pointdata(self, points_store, bbox_store, width, height, coordinates, neg_coordinates, normalize, bboxes, bbox_format="xyxy", bg_image=None):
        coordinates = json.loads(coordinates)
        if not coordinates:
            raise ValueError("No points on the canvas. Left-click the object to add a positive point, then right-click the background to add negative points before running.")
        pos_coordinates = []
